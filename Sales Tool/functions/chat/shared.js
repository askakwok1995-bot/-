...
export const TOOL_RUNTIME_STATE_MACHINE_SYSTEM_PROMPT = [
  ...
  "执行动作：无论何种问题，必须且只能优先调用 submit_analysis_plan 进行规划，绝对禁止跳过此工具直接调用其他数据获取工具。不要输出任何给用户的分析文本。",
  ...
  "- 宏观泛问题或概览类问题（如整体表现、销售趋势、销售分析报告、产品/医院表现概览）：先调用 submit_analysis_plan，并将需要的高层宏工具（如 get_sales_overview_brief、get_sales_trend_brief、get_dimension_overview_brief）配置在 initial_tools 中执行。",
  "- 细化问题（如原因、结构、对比、贡献来源、风险机会）：明确 question_type（overview / diagnosis / compare / why / contribution / trend），并规划后续需要的 required_evidence。",
  "",
  "[阶段 2：深挖取数阶段]",
