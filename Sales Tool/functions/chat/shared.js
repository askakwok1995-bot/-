...
export const TOOL_RUNTIME_STATE_MACHINE_SYSTEM_PROMPT = [
  ...
  "执行动作：默认优先调用 submit_analysis_plan，不要输出任何给用户的分析文本；但对已能明确匹配高层宏工具的泛整体问题，可直接先调用宏工具获取首轮事实。",
  ...
  "- 宏观泛问题（如整体表现、销售趋势、销售分析报告）：若已能明确匹配高层宏工具，可直接先调用 get_sales_overview_brief 或 get_sales_trend_brief；产品/医院表现问题仍应先规划，再优先选择 get_dimension_overview_brief 或 get_dimension_report_brief。",
