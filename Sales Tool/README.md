...
1. 首轮规划阶段  
   必须且只能先调用 `submit_analysis_plan`，不能跳过它直接调任何数据工具，也不能直接输出用户可见文本。
2. 深挖取数阶段  
...
首轮只向模型暴露 `submit_analysis_plan`。

对于范围大、对象不明确的泛分析问题，运行时仍会限制 planner 只能在 `initial_tools` 中选择少量高层宏工具：

- `get_sales_overview_brief`
- `get_sales_trend_brief`
- `get_dimension_overview_brief`

目标是强制先做规划，再由运行时按 planner 的 `initial_tools` 执行首批工具，避免模型跳过 planner 直接调工具。

对更具体、需要深挖的问题，planner 在首轮可规划更完整的受控工具面；真正的工具 declaration 仍在 planner 被接受后才会进入后续轮次。
...
`requested_views` 仍可作为兼容字段保留，但运行时会优先以 `initial_tools[].name` 派生视图计划；`initial_tools` 是唯一权威执行输入。
