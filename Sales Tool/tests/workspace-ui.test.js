...
test("applyWorkspaceReadOnlyState 会锁定控件、展开卡片并展示演示横幅", () => {
  ...
  try {
    ...
    applyWorkspaceReadOnlyState(
      {
        ...
        workspaceModeBannerKickerEl: kicker,
    ...
    assert.equal(kicker.textContent, "演示工作台");
...
test("applyWorkspaceReadOnlyState 在非演示只读态展示授权横幅", () => {
  const env = installFakeBrowserEnv();
    ...
    applyWorkspaceReadOnlyState(
      ...
      },
      {
        ...
        workspaceBanner: {
          kicker: "使用授权",
          title: "当前账号授权已到期",
          description: "当前账号授权已失效，请联系管理员续费。",
        },
      },
    );

    assert.equal(banner.hidden, false);
    assert.equal(kicker.textContent, "使用授权");
    assert.equal(title.textContent, "当前账号授权已到期");
    assert.equal(desc.textContent, "当前账号授权已失效，请联系管理员续费。");
    assert.equal(root.dataset.workspaceMode, "live");
  } finally {
    env.restore();
  }
...
test("applyWorkspaceReadOnlyState 在演示态保留图表分析控件可交互", () => {
  const env = installFakeBrowserEnv();
  ...
  try {
    ...
    const details = new FakeHtmlDetailsElement(summary);
    const root = new FakeElement();
    const lockedInput = new FakeHtmlInputElement();
    const lockedButton = new FakeHtmlButtonElement();
    const chartMetricButton = new FakeHtmlButtonElement();
    const chartSelect = new FakeHtmlSelectElement();

    applyWorkspaceReadOnlyState(
      {
        workspaceGrid: root,
        workspaceLockedControls: [lockedInput, lockedButton],
        workspaceControls: [lockedInput, lockedButton, chartMetricButton, chartSelect],
        workspaceDetails: [details],
      },
      {
        isWorkspaceReadOnly: true,
    ...
    assert.equal(chartMetricButton.disabled, false);
    assert.equal(chartSelect.disabled, false);
  } finally {
    env.restore();
  }
});
