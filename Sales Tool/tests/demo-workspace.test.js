...
test("createDemoWorkspaceSnapshot 生成可用于演示工作台的完整数据", () => {
  ...
  assert.equal(snapshot.products.length >= 4, true);
  assert.equal(snapshot.records.length > 0, true);
...
test("createDemoWorkspaceSnapshot 的数据可直接渲染为非空报表和图表", () => {
  ...
  try {
    ...
    assert.match(dom.reportProductBody.innerHTML, /瑞舒伐他汀/);
    assert.match(dom.reportHospitalBody.innerHTML, /上海瑞金医院/);
